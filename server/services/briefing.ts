...
function monthLine(db: DB, month: string): string {
  ...
  const detail = cats.map((c) => `${c.name} ${da(c.total)}`).join(", ");
...
export function financialBriefing(db: DB, today = todayIso()): string {
  ...
  const lines: string[] = [`Date ${today}. Montants en dinars.`];

  const wallets = listWallets(db);
  if (wallets.length > 0) lines.push(`Soldes: ${wallets.map((w) => `${w.name} ${da(w.balance)}`).join(", ")}`);
  ...
  if (budgets.length > 0) lines.push(`Budgets ${month} (prévu/dépensé): ${budgets.map((b) => `${b.categoryName} ${da(b.amount)}/${da(b.spent)}`).join(", ")}`);

  const projects = listProjects(db, today);
  if (projects.length > 0) {
    lines.push(`Projets: ${projects.map((p) => `${p.name} objectif ${da(p.target)}, épargné ${da(p.saved)}${p.dueDate ? `, pour ${p.dueDate}` : ""}${p.monthlyNeeded !== null ? `, ${da(p.monthlyNeeded)}/mois` : ""}`).join(" | ")}`);
  }

  const bills = upcomingBills(db, today, addDays(today, 30));
  if (bills.length > 0) lines.push(`Échéances 30j: ${bills.map((b) => `${b.date.slice(5)} ${b.label} ${da(b.amount)}`).join(", ")}`);
  ...
  if (insights.length > 0) lines.push(`Constats: ${insights.map((i) => `${i.title} (${i.text})`).join(" ")}`);
